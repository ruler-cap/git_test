...
在真机控制能力上，项目已经实现了基于关节坐标和末端笛卡尔坐标的两类主要控制方式。关节坐标控制可用于姿态保持、轨迹复现、单关节调试和基础 bring-up；末端坐标控制可直接面向双手操作任务，通过给定左右手和躯干目标位姿实现更符合任务语义的控制。同时，系统在真实硬件上打通了位置控制、速度控制和力矩控制三类输出路径：身体关节主要通过 CSP 位置控制实现稳定跟踪，移动底盘通过 CSV 速度控制实现运动，手臂在 MIT/direct-CST 场景下可通过 CST 力矩控制实现顺应交互。这些能力共同构成了后续复杂机器人控制、移动操作和环境交互的基础。

在真机硬件尚未具备完整验证条件的早期阶段，项目首先在 `python/` 中建立了参考实现和 MuJoCo 仿真验证环境。该仿真环境不是单纯的算法脚本，而是尽量按照真机部署边界设计：通过 `RobotAdapter` 抽象统一仿真和真实机器人读写接口，通过 `MotorSDK.state_get()` / `MotorSDK.state_update()` 模拟 24 电机 SDK 的状态读取和命令写入，通过 `TaskCommand`、`RobotState`、`ControlCommand` 等中间数据结构隔离 ROS、控制器和 SDK。`SimMotorSDK` 在 MuJoCo 中复现真实机器人 4 个底盘轮和 20 个身体关节的 SDK 数组接口，使仿真阶段的控制链路、数据格式、命令模式和真机部署路径保持一致。

基于该仿真接口，项目在没有真机或真机验证风险较高时，先通过 Python runners、MuJoCo plant、离线 CSV replay 和单元测试完成算法验证。仿真中不仅验证 WBC 任务能否求解，还验证底盘 mecanum 映射、motor SDK marshalling、任务源解码、安全过滤、数据录制格式以及 C++/Python replay 对拍。后续 C++ 和 `output_project` 的真机部署是在该 Python 参考实现基础上逐步移植和交叉验证完成的，因此形成了从仿真参考、C++ 移植、离线对拍到真机运行的连续迁移路径。
...
考虑到全身控制直接作用于真实机器人存在较高风险，项目从设计阶段就采用了分级递进的 WBC 验证路线。早期仿真配置和后续真机 mode 均按“由简单到复杂”的原则组织：先验证固定底盘下的双臂/关节空间控制，再加入腰部和躯干任务，再加入 CoM 约束和自碰撞保护，最后扩展到底盘参与的全身控制以及 CST 力矩顺应控制。真机验证时同样按照该能力阶梯逐级 bring-up，每一级只增加有限的新任务和风险源，先确认低风险 CSP 位置路径，再验证底盘 CSV，最后再进入 MIT/direct-CST 等高风险力矩路径。
...
11. **仿真接口与真机接口同构设计**
    ...
    项目早期在没有完整真机条件时，已在 Python/MuJoCo 环境中按照真机部署边界设计仿真接口。`RobotAdapter` 抽象统一仿真和真实机器人读写，`SimMotorSDK` 按 24 电机 SDK 数组模拟 `state_get/state_update`，deployment interface 使用与真机一致的任务命令、机器人状态和控制命令结构。该设计使仿真验证不只是“看算法能不能跑”，而是提前验证了控制链路、SDK 数据格式、底盘轮速映射、ROS/无 ROS 入口和安全过滤接口，为后续真机迁移降低了大量接口重构风险。

12. **分级递进式 WBC 安全验证路线**

    项目将 WBC 任务按风险和复杂度分层设计，并在仿真和真机验证中沿用同一递进路线。控制能力从固定底盘、关节/双臂末端跟踪开始，逐步增加躯干、CoM、自碰撞、移动底盘，最后进入 MIT 和 direct-CST 力矩顺应控制。每一级验证只引入有限的新变量，能够明确定位问题来源，并在进入更高风险功能前确认低层控制和安全门控可靠。这种分级验证方法尤其适合复杂机器人从仿真走向真机的技术转化过程。

13. **关节坐标、末端坐标与多控制模式统一打通**

    项目在真机上实现了基于关节坐标和末端笛卡尔坐标的控制能力，并进一步打通位置控制、速度控制和力矩控制三类硬件输出模式。关节坐标控制为姿态保持、轨迹复现和单关节调试提供基础；末端坐标控制使机器人能够以任务空间方式控制双手和躯干，更适合移动操作任务；位置控制保证常规运动的稳定性，速度控制支撑移动底盘运动，力矩控制则为顺应控制和外部交互提供基础。尤其是力矩控制路径的建立，使机器人在与人、物体和环境接触时能够从刚性位置跟踪转向可让步的力控/顺应行为，为后续安全交互、柔顺操作和复杂作业任务奠定了关键基础。
